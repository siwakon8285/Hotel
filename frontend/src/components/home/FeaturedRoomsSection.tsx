          ...
          {featured.map((room, index) => {
            ...
            return (
              <Reveal key={room.id} animation="fade-up" delay={index * 0.1}>
                ...
                >
                  {/* Image Side */}
                  <div className="w-full lg:w-3/5 overflow-hidden relative aspect-[4/3] bg-zinc-900">
                    <Image
                      src={imgSrc}
                      alt={room.name}
                      fill
                      sizes="(max-width: 1024px) 100vw, 60vw"
                      className="object-cover transition-transform duration-1000 group-hover:scale-103 opacity-90 group-hover:opacity-100"
                    />
                  </div>
